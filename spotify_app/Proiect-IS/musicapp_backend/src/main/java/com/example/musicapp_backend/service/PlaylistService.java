...
@Service
@Transactional // 1. CRITICAL: Ensures data integrity and handles lazy loading
public class PlaylistService {
    ...
    /**
     * Renamed from 'searchByUser' to 'findAllByUserId' to match Controller expectations
     * and standard Spring naming conventions.
     */
    @Transactional(readOnly = true)
     ...
     * This is the preferred method when creating playlists from authenticated endpoints.
     * Note: principal.getName() returns the username from JWT token.
     */
    ...
    public PlaylistDto update(Long id, PlaylistCreateRequest req) {
        ...
        // 2. CHANGE: Do NOT update the User (Ownership) here.
    ...
    public void delete(Long id) {
        ...
        // 3. NOTE: Because we have @Transactional now, this lazy-loaded
        // 'getSongs()' call is safe and won't throw LazyInitializationException.
    ...
    public PlaylistDto addSong(Long playlistId, Long songId) {
        ...
        playlist.addSong(song); // Helper method in Entity handles the bidirectional link

        Playlist updated = playlistRepo.save(playlist);
        return toDto(updated);
    }

    // Renamed to 'removeSong' to match Controller
    public PlaylistDto removeSong(Long playlistId, Long songId) {
        ...
        playlist.removeSong(song); // Helper method in Entity handles the bidirectional link
